...
st.sidebar.image("D:\Dicoding\DBS Foundation\Submission\Proyek Visualisasi Data\dashboard\logo_proyek.jpeg")
...
if page == "Page 1 : Preview Dataset & Visualisasi musim serta cuaca":
    ...
    sns.barplot(x=filtered_df['season'], y=filtered_df['cnt'], ax=ax, palette="coolwarm")
    ax.set_xlabel("Musim")
    ...
    fig, ax = plt.subplots(figsize=(4, 3))
    sns.barplot(x=filtered_df['weathersit'], y=filtered_df['cnt'], ax=ax, palette="viridis")
    ax.set_xlabel("Cuaca")
    ax.set_ylabel("Total Peminjaman")
    ax.set_xticklabels(["Clear", "Cloudy", "Rain"], rotation=45)
    st.pyplot(fig)
    ...
    day_df = pd.read_csv("D:\Dicoding\DBS Foundation\Submission\Proyek Visualisasi Data\data\day.csv")  # Pastikan file tersedia
    hour_df = pd.read_csv("D:\Dicoding\DBS Foundation\Submission\Proyek Visualisasi Data\data\hour.csv")
